...
function tfxMkboot(options, ...commands) {
    ...
    var oldBootFile = '';
    try {
        oldBootFile = getElement(taskJson, 'execution.Node10.target', taskJsonFile, 'Task', (obj) => taskJson = obj);
    } catch(err) {
...
//#endregion
    ...
    //#region Define new boot file
    bootJs = `const path = require('path');
...
var rwd = process.env[rwdEnv];
if (!rwd) {
    console.info(\`Environment variable '\${rwdEnv}' could not be found.\`);
} else {
    twd = path.join(rwd, '_tasks', '${name}_${id}', '${pver}');
}

console.log(\`Bootstrapping task '${name}' ...\`);
console.debug(\`node version: \${process.version} (executable: \${process.execPath})\`);
console.debug(\`node cwd:     '\${process.cwd()}'\`);
console.debug(\`task folder:  '\${twd}'\`);

var npmPaths = [];
try {
    cp.execSync('npm install npm --no-save', { cwd: twd, stdio: 'inherit' });
...
if (npmPath && typeof(npmPath) === 'string') {
    try {
        require('fs').readdirSync(path.join(npmPath, 'node_modules')).forEach(de => {
            console.debug(de);
        });
    } catch(err) {
        console.error(err.message || err);
...
for (var i=0; i < npmPaths.length; i++) {
    npmPath = npmPaths[i];
    try {
        console.debug(\`Trying [require('\${npmPath}')] ...\`);
        npm = require(npmPath);
...
console.debug(\`npm version: v\${npm.version}\`);
console.log(\`initiating package load ...\`);
npm.load({ production: true, loglevel: 'silly', prefix: twd }, function(err) {
    
    if (err) {
        console.error(err.message);
        console.log('Trying install anyway.');
    } else {
...
console.debug(\`leaving '\${__filename}' .\`);
...
function tfxVersion(options) {
    ...
    if (extensionNeedsUpdate) {
        ...
        var vsixPackageVersionFile = path.join(path.dirname(options.vsixPackageFile), `${publisher}.${id}-${extensionJson.version}.json`);
        versionNeedsUpdate = true;
