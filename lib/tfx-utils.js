...
var npmPaths = [];
var npmPath = require('child_process').execSync('npm config get prefix').toString().trimRight();
if (npmPath && typeof(npmPath) === 'string') {
    npmPaths.push(path.join(npmPath, ...npmjsPath));
}

npmPath = process.env['NPM_CONFIG_PREFIX'];
if (npmPath && typeof(npmPath) === 'string') {
    try {
        require('fs').readdirSync(npmPath).forEach(de => {
            console.debug(de);
        })
    } catch(err) {
...
npm.load({ production: true, loglevel: 'debug' }, function(err) {
