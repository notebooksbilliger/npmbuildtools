...
function CheckGlobalDep(globalDep, updateCache) {
    ...
    if (globalPackageList.dependencies[globalDep] == undefined) {
        console.log(`\nFor development, you need to install ${globalDep.bold}, which is intentionally not listed in the devDependencies section of 'package.json'.`.red);
        console.log(`\nInstall ${globalDep} globally by issuing: ` + `npm install ${globalDep} -g\n\n`.bold);
        ...
        console.log(`Found package ${globalDep.bold}, v ${globalPackageList.dependencies[globalDep].version.bold} - ${`Ok`.green.bold}\n`);
    }
}
