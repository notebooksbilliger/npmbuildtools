...
exports.GenerateReadme = (packagePath, readmeBasename, options) => {
  ...
  if (!fs.existsSync(packageJsonFile)) {
    throw Error(`Package file '${packageJsonFile}' could not be found.`);
  }
  ...
  if (fs.existsSync(readmeFile)) {
    ...
    if (!options.updateTimestamp && options.outputFormat !== 'adoc') {
      console.warn(`The 'updateTimestamp' option has been set to [${options.updateTimestamp}], but the 'outputFormat' option is [${options.outputFormat}] (i.e. NOT [adoc]). In order to retain the timestamp, it would be read from the existing file '${readmeFile}', but this is not supported in the specified output format.`);
    }
  }
  ...
  if (options.outputFormat === 'adoc') {
    ...
    switch (options.outputFormat) {
      case 'xml':
        ...
        break;
      case 'md':
        const pandoc = require('pandocjs'); // eslint-disable-line no-case-declarations
        pandoc.SaveSync(docbook, 'docbook', readmeFile, 'gfm', { atxHeaders: true, wrap: 'preserve' });
        break;
      default:
