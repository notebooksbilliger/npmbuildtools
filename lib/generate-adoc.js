...
exports.GenerateReadme = function GenerateReadme (packagePath, readmeFileName, options) {
  ...
  var attributes = {};
  fs.writeFileSync(readmeFile, resolve({ includeRootSourcePath: path.dirname(readmeFile), EOL: options.EOL, attributes: attributes }, ...adoc), { encoding: 'utf8' });
  fs.appendFileSync(readmeFile, options.EOL.repeat(1), { encoding: 'utf8' });
  fs.appendFileSync(readmeFile, resolve({ includeRootSourcePath: path.dirname(readmeFile), EOL: options.EOL, attributes: attributes }, ...adocTxt), { encoding: 'utf8' });

  if (readmeBodyStat) {
    var readmeBody = readLinesSync(readmeBodyFile, { encoding: 'utf8' });
    fs.appendFileSync(readmeFile, options.EOL.repeat(2), { encoding: 'utf8' });
    fs.appendFileSync(readmeFile, resolve({ includeRootSourcePath: path.dirname(readmeBodyFile), EOL: options.EOL, attributes: attributes }, ...readmeBody), { encoding: 'utf8' });
  }
