...
exports.GenerateReadme = function GenerateReadme(packagePath, readmeFileName, lineBreak, updateTimestamp) {
    ...
    if (updateTimestamp) {
        ...
        if (!isNaN(value)) {
            console.debug(`Using timestamp of last commit of '${readmeBodyFile}'.`);
            value = new Date(value);
        } else {
            value = getLastCommitTimestamp(readmeFile, undefined, 'D');
            if (!isNaN(value)) {
                console.debug(`Using timestamp of last commit of '${readmeFile}'.`);
                value = new Date(value);
            } else {
                if (readmeBodyStat) {
                    console.debug(`Using timestamp of last modification of '${readmeBodyFile}'.`);
                    value = readmeBodyStat.mtime;
                } else {
                    if (readmeStat) {
                        console.debug(`Using timestamp of last modification of '${readmeFile}'.`);
                        value = readmeStat.mtime;
                    } else {
                        console.debug(`Defaulting to timestamp of current 'Date()'.`);
                        value = new Date();
                    }
                }
            }
