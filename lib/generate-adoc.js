 ...
 * - Currently supported status codes are limited to `A`, `M`, `D` and 'MM' - even
 ...
 * - Otherwise, omit or set to `undefined`.
 * - If `undefined`, default to: `--no-pager show --name-status --format=%cd --date=iso-strict --reflog -- "${pathSpec}"`
 ...
 * @param {string[]} ignoreStatusCodes A list of status code to ignore.
 */
function getLastCommitTimestamp(pathSpec, customCommand, excludeStatusCodes, ignoreStatusCodes) {
    if (!customCommand) {
        customCommand = `--no-pager show --name-status --format=%cd --date=iso-strict --reflog -- "${pathSpec}"`;
    }
    ...
    if (!result) {
        ...
        result.forEach(line => {
            ...
            if (!isNaN(timestamp)) {
                ...
                historyEntry = new HistoryEntry(timestamp);
            } else {
                if (!historyEntry) {
                    throw Error(`Unable to parse result:\n${result.join('\n')}`);
                }
                statusLine = line.split('\t', 2);
                statusEntry = new StatusEntry(statusLine[0], statusLine[1]);
...
exports.GenerateReadme = function GenerateReadme(packagePath, readmeFileName, lineBreak, updateTimestamp) {
    ...
    if (updateTimestamp) {
        value = getLastCommitTimestamp(readmeBodyFile, undefined, ['D'], ['MM']);
        if (!isNaN(value)) {
            value = new Date(value);
            console.debug(`Using timestamp of last commit of '${readmeBodyFile}': ${value.toString()}`);
        } else {
            value = getLastCommitTimestamp(readmeFile, undefined, ['D'], ['MM']);
            if (!isNaN(value)) {
                value = new Date(value);
...
exports.GetLastCommitTimestamp = function GetLastCommitTimestamp(path, customCommand, excludeStatusCodes, ignoreStatusCodes) {
    if (!path || !(typeof(path) === 'string')) {
        throw Error(`'${path}' is not a valid path specification for parameter 'path'.`);
    }
    return getLastCommitTimestamp(path, customCommand, excludeStatusCodes, ignoreStatusCodes);
}