 ...
 * @param { { declarationFileExtension?: string, dryRun?: boolean, consoleOptions?: import('../index').ConsoleOptions|import('../index').ConsoleOptions_ }= } options
...
function removeDeclarations (packagePath, options, ...includeSubfolders) {
  if (includeSubfolders.length < 1) {
    ...
    if (packageJson.directories) {
      Object.keys(packageJson.directories).forEach(dir => {
        includeSubfolders.push(packageJson.directories[dir]);
      });
    }
  }
